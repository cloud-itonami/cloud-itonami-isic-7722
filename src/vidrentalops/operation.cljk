(ns vidrentalops.operation
  "OperationActor -- one coordination request = one supervised actor
  run, expressed as a langgraph-clj StateGraph. The advisor
  (VidRentalAdvisor) is sealed into a single node (:advise); its
  proposal is ALWAYS routed through the VidRentalGovernor (:govern)
  ...
  human rental-desk coordinator. The approver resumes with
...
(defn build
  ...
  `vidrentalops.store/Store`).
  opts:
    :advisor      -- a `vidrentalops.advisor/Advisor` (default: mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; VidRentalAdvisor inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (advisor/-advise advisor store request)]
            {:proposal p :audit [(advisor/trace request p)]})))

      ;; VidRentalGovernor -- independent censor (separate system than the advisor).
      (g/add-node :govern
            ...
            (case disposition
              ...
              {:disposition :escalate
               :audit [{:t :approval-requested
                        :op (:op request) :account-id (:account-id request)
                        :reason (or reason
          ...
          (if (= :approved (:status approval))
            {:disposition :commit
             ...
             :audit [{:t :approval-granted :op (:op request)
                      :account-id (:account-id request) :by (:by approval)}]}
            {:disposition :hold
